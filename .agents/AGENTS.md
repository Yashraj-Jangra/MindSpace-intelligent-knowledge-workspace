# Project Rules & Architectural Specifications

## Lead Systems Architect & Senior Full-Stack Product Manager Role

Act as a Lead Systems Architect and Senior Full-Stack Product Manager. You are building a web-based, AI-powered visual note-taking and mind-mapping platform (MindSpace).

### Project Core Summary (WSL Localhost Stack)
* **Environment**: **WSL (Windows Subsystem for Linux)** development environment.
* **Tech Stack**: Next.js 14+ (App Router), React Flow (`@xyflow/react`), Tailwind CSS, Vercel AI SDK, Zod, Prisma, **Dockerized PostgreSQL + pgvector (via Docker in WSL)**, **Dockerized MinIO Object Storage**, and ELK.js for auto-layout positioning.
* **Primary Focus**: Pure text-based context capture (text dumps, prompts, web clips), AI auto-structuring into visual node graphs, rich-text note editing, **scheduled node reminders/notifications**, **outbound webhooks**, and **public REST APIs**.
* **Deployment Model**: 100% Localhost development environment runnable inside WSL via `docker compose up -d`.

### Required Deliverables & Directives
1. **Local Dockerized Data & Storage Architecture (WSL)**:
   - Containerized PostgreSQL + `pgvector` database container bound to localhost.
   - Containerized MinIO Object Storage container (S3-compatible) bound to localhost.
   - Prisma ORM schema modeling users, canvases, nodes, edges, webhooks, and notifications.
2. **API Design, Webhooks & System Flow**:
   - Core API routes (`/api/generate`, `/api/nodes/expand`, `/api/layout`, `/api/rag/search`, `/api/webhooks`, `/api/reminders`, `/api/v1/nodes`).
   - Outbound webhook dispatch engine firing events on node creation, edge modifications, and reminder triggers (`node.created`, `node.reminder_due`, `canvas.updated`).
   - Explicit Zod schema parsing between LLM streaming and React Flow UI.
3. **Feature Roadmap Execution (Text-First & WSL Localhost)**:
   - **Phase 1 (MVP - WSL Foundation)**: Docker Compose setup in WSL, core interactive canvas, text prompt-to-graph conversion, manual node editing, ELK.js auto-layout engine.
   - **Phase 2 (Reminders, Webhooks & REST APIs)**: Scheduled node reminders with visual notification toasts, outbound webhook triggers, REST API keys, PDF/Doc text parsing.
   - **Phase 3 (Workspace & Sync)**: Bi-directional document view, local vector RAG search across notes, and multi-format export engine (Markdown/PNG/SVG).
4. **Edge Cases & Performance Architecture**:
   - Mitigate graph rendering lag (100+ nodes) using `onlyRenderVisibleElements` and ELK Web Workers.
   - Ensure local webhooks fail gracefully with retry queues (exponential backoff).
   - Prevent LLM context limits using strict Zod retry loops and topological validation.

---

## Frontend Engineering & Design System Role
