...
- [x] **Stylus Input, Scroll & Double-Click Fixes** (Session 2026-07-25 v5):
  ...
  - **Root Cause: Double-Click Creates Annotation**: Added `lastPointerDownTimeRef` and `lastPointerDownPosRef` to track the last down event timestamp and position. If a second pointer-down occurs within 300ms and within 20px of the first, it is classified as a double-tap and suppressed. Added `onDoubleClick` with `preventDefault + stopPropagation` on the canvas element and workspace container to block browser-level zoom/selection.
  - **Desktop vs. Tablet Input Discrimination**: `isTouchDeviceRef` set once at mount via `navigator.maxTouchPoints > 0`. Desktop (maxTouchPoints=0): `mouse` events draw like a pen. Tablet: only `pen` pointer type draws; `touch` always scrolls.
  - **Stroke Commit on Leave**: Replaced the simple `onPointerLeave` cleanup with a full `commitCurrentStroke()` call, so any stroke in progress is not lost if the stylus leaves the canvas boundary mid-stroke (e.g. near edge of tablet screen).
  - **`onPointerCancel` Handler**: Added to handle OS interruptions (incoming call, app switch) that fire `pointercancel` instead of `pointerup`, ensuring strokes are always committed cleanly.
  - **Auto-Detected `stylusOnlyMode` Default**: Changed `AdvancedNoteEditor` initial state to auto-detect the device type and set `stylusOnlyMode: true` on tablets by default, so users don't need to configure anything for correct pen+scroll behaviour out of the box.
