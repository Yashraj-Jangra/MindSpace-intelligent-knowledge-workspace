...
import { ListTodo, Plus, Search, ShieldAlert, Sparkles } from "lucide-react";
...
export default function TasksPage() {
  ...
  const [filter, setFilter] = useState<"ALL" | "ACTIVE" | "DONE" | "CRITICAL">(
    "ALL",
  );
        ...
        prev.map((macro) => {
          if (macro.subtasks) {
            ...
            if (hasSub) {
              ...
              const completed = nextSubs.filter(
                (s) => s.status === "DONE",
              ).length;
  ...
  const handleDeleteMacro = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this macro objective and all its subtasks?",
      )
    )
      return;
        ...
        actions={
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#141414] hover:bg-[#1A1A1A] border border-[#FF3D00] text-[#FAFAFA] font-mono text-xs uppercase transition-colors"
            >
              <Sparkles className="w-3.5 h-3.5 text-[#FF3D00]" />
              <span className="hidden sm:inline">AI Goal Breakdown</span>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 sm:p-8 space-y-6 sm:space-y-8">
        ...
        {isFormOpen && (
          <form
            ...
            className="bg-[#0F0F0F] border border-[#FF3D00] p-6 relative space-y-4"
          >
            <div className="h-1 w-12 bg-[#FF3D00] absolute top-0 left-0" />
            <h3 className="font-mono text-xs uppercase text-[#FF3D00] font-bold">
              New Macro Objective
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="font-mono text-[10px] uppercase text-[#737373] block">
                  Objective Title
                </label>
                <input
                  type="text"
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <label className="font-mono text-[10px] uppercase text-[#737373] block mr-3">
                  Priority Level
                </label>
                <select
                  value={newPriority}
        ...
        {isLoading ? (
          ...
          <div className="bg-[#0F0F0F] border border-[#262626] p-12 text-center space-y-4">
            ...
            <h3 className="font-sans font-bold text-lg uppercase text-[#FAFAFA]">
              No Objectives Found
            </h3>
            <p className="text-xs font-mono text-[#737373] max-w-xs mx-auto">
              Create a macro objective and break it down into micro subtasks to
              track execution.
            </p>
          </div>
        ) : (
