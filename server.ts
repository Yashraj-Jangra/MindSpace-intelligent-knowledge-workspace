...
app
  ...
  .then(async () => {
    ...
    try {
      await import("./src/workers/reminder.worker");
      await import("./src/workers/embedding.worker");
      console.log(
        "[Workers Manager] BullMQ reminder & embedding workers initialized.",
      );
    } catch (workerErr) {
      console.warn(
        "[Workers Manager Warning]: Failed to boot background workers:",
    ...
    async function startDiscordBot() {
      try {
        ...
        const statusText =
          (await getSystemSetting("DISCORD_BOT_STATUS")) ||
          "Listening to /remind";

        if (!botToken) {
          console.log(
            "[Discord Bot Manager] No bot token configured. Waiting...",
          );
          if (discordClient) {
        ...
        console.log(
          "[Discord Bot Manager] Attempting persistent login to Gateway...",
        );
        ...
        discordClient.on("interactionCreate", async (interaction) => {
          ...
          const payload = {
            ...
            member: interaction.member
              ...
              : undefined,
            data: {
              id: interaction.isChatInputCommand()
              ...
              component_type: interaction.isButton()
                ? 2
                : interaction.isStringSelectMenu()
                  ? 3
                  : undefined,
          ...
          try {
            ...
            const response = await fetch(
              `http://localhost:${port}/api/discord/bot`,
              {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
              ...
              await interaction.editReply(
                `❌ Failed to process command on local server: ${errText || "Internal error"}`,
              );
            }
          } catch (err) {
            ...
            try {
              await interaction.editReply(
                "❌ Local server connection error or processing timeout.",
              );
            } catch (replyErr) {
              console.warn(
                "[Discord Bot Gateway Reply Error]:",
        ...
        discordClient.once("ready", () => {
          console.log(
            `[Discord Bot Manager] Persistent bot ONLINE: ${discordClient?.user?.tag}`,
          );
        ...
        console.error(
          "[Discord Bot Manager] Persistent connection failed:",
    ...
    async function startTelegramBot() {
      try {
        ...
        if (!botToken) {
          console.log(
            "[Telegram Bot Manager] No bot token configured. Waiting...",
          );
          if (telegramBot) {
        ...
        console.log(
          "[Telegram Bot Manager] Attempting persistent login via Long Polling...",
        );
        ...
        telegramBot.command("pair", async (ctx) => {
          ...
          if (!code) {
            return ctx.reply(
              "Please specify a pairing code, e.g. <code>/pair 123456</code>",
              { parse_mode: "HTML" },
            );
          }
          try {
            const { pairTelegramAccount } = require("./src/lib/telegram/bot");
            const account = await pairTelegramAccount(
              code,
              ctx.chat.id.toString(),
              ctx.from?.username,
            );
            const escapedUsername = (account.username || "user")
              .replace(/&/g, "&amp;")
              .replace(/</g, "&lt;")
              .replace(/>/g, "&gt;");
            ctx.reply(
              `✅ <b>Account successfully paired!</b>\nWelcome to MindSpace, ${escapedUsername}!`,
              { parse_mode: "HTML" },
            );
          } catch (err) {
            const escapedError = ((err as Error).message || "")
              .replace(/&/g, "&amp;")
              .replace(/</g, "&lt;")
              .replace(/>/g, "&gt;");
            ctx.reply(`❌ <b>Pairing failed:</b> ${escapedError}`, {
              parse_mode: "HTML",
            });
          }
        });
        ...
        telegramBot.command("tasks", async (ctx) => {
          ...
          if (!userId)
            return ctx.reply(
              "❌ Your Telegram account is not paired. Please run `/pair <code>` first.",
            );
        ...
        telegramBot.command("done", async (ctx) => {
          ...
          if (!taskId) {
            return ctx.reply(
              "Please specify the task ID, e.g. \`/done <id>\`",
              { parse_mode: "Markdown" },
            );
          }
          ...
          try {
            ...
            ctx.reply(`✅ Task **${task.title}** marked as done!`, {
              parse_mode: "Markdown",
            });
          } catch (err) {
            console.error("[Telegram Bot /done Error]:", err);
            ctx.reply(
              "❌ Failed to complete task. Make sure the task ID is correct.",
            );
          }
        });
        ...
        telegramBot.command("capture", async (ctx) => {
          ...
          if (!text) {
            return ctx.reply(
              "Please specify text to capture, e.g. \`/capture Read research paper\`",
              { parse_mode: "Markdown" },
            );
          }
          ...
          ctx.reply("📥 Saved message to your **Quick Capture Inbox**!", {
            parse_mode: "Markdown",
          });
        });
        ...
        telegramBot.command("remind", async (ctx) => {
          ...
          if (!arg) {
            return ctx.reply(
              "Format: \`/remind message | when\`\nExample: \`/remind Call team | tomorrow at 3pm\`",
              { parse_mode: "Markdown" },
            );
          }
          ...
          if (arg.includes("|")) {
            ...
            if (parts.length >= 2) {
              ...
              return ctx.reply(
                "Could not parse arguments. Please use `|` to separate details and date.",
                { parse_mode: "Markdown" },
              );
            }
          }
          ...
          if (!scheduledFor) {
            return ctx.reply(
              `❌ Could not parse date/time offset: \`${whenPhrase}\``,
              { parse_mode: "Markdown" },
            );
          }
        ...
        telegramBot.command("add", async (ctx) => {
          ...
          await redis.setex(
            sessionKey,
            300,
            JSON.stringify({ userId, step: "WAITING_TITLE" }),
          );
          ctx.reply(
            "📋 **Conversational task builder started.**\nPlease enter the task title:",
          );
        });
        ...
        telegramBot.on("message:text", async (ctx) => {
          ...
          if (session.step === "WAITING_DUE") {
            ...
            if (!isSkip && !dueAt) {
              return ctx.reply(
                '❌ Could not parse date format. Please try again or type "skip":',
              );
            }
          ...
          if (session.step === "WAITING_PRIORITY") {
            ...
            if (!validPrios.includes(prio)) {
              return ctx.reply(
                "❌ Invalid priority. Please type LOW, MEDIUM, HIGH, or CRITICAL:",
              );
            }
        ...
        telegramBot.api
          ...
          .then((me) => {
            console.log(
              `[Telegram Bot Manager] Bot logged in and ONLINE as @${me.username}`,
            );
          })
          .catch((e) => {
            console.error(
              "[Telegram Bot Manager] Failed to load metadata:",
              e.message,
            );
          });
      } catch (err) {
        console.error(
          "[Telegram Bot Manager] Long polling start failed:",
          (err as Error).message,
        );
      }
    }
    ...
    subClient.subscribe("socket-emit", (err) => {
      if (err) {
        console.error(
          "[Socket.io Redis Sub] Failed to subscribe to socket-emit:",
          err,
        );
      } else {
      ...
      if (channel === "socket-emit") {
        try {
          ...
          if (event === "settings:updated") {
            console.log(
              "[Socket.io Redis Sub] Settings updated. Re-initiating Discord & Telegram Bot Gateways...",
            );
            startDiscordBot();
            startTelegramBot();
          }
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("node:lock", (data) => {
        if (data?.canvasId && data?.nodeId) {
          socket.to(`canvas:${data.canvasId}`).emit("node:lock", {
            nodeId: data.nodeId,
            user: data.user,
            lockedAt: Date.now(),
          });
      ...
      socket.on("node:unlock", (data) => {
        if (data?.canvasId && data?.nodeId) {
          socket.to(`canvas:${data.canvasId}`).emit("node:unlock", {
            nodeId: data.nodeId,
            userId: data.user?.id,
          });
        }
        ...
        if (userId) {
          ...
          if (!stillConnected) {
            try {
              ...
              console.error(
                "Failed to remove user from online users in Redis:",
                err,
              );
            }
          }
