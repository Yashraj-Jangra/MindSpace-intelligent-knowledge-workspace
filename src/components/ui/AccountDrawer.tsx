...
export function AccountDrawer({ isOpen, onClose, user }: AccountDrawerProps) {
  ...
  // AI & Privacy Toggle State
  const [isAiEnabled, setIsAiEnabled] = useState(true);

  useEffect(() => {
    const saved = localStorage.getItem("mindspace-ai-enabled");
    if (saved !== null) {
      setIsAiEnabled(saved !== "false");
    }
  }, []);
  ...
  const handleToggleAi = () => {
    ...
    localStorage.setItem("mindspace-ai-enabled", String(nextVal));
    window.dispatchEvent(
      new CustomEvent("mindspace:ai-toggle", { detail: { enabled: nextVal } }),
    );
  };
  ...
  const whEvents = ["node.created", "reminder.fired"];
  ...
  const handleGenerateTelegramCode = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setDcSuccess(
          "Telegram pairing code generated. Send `/pair <code: " +
            data.code +
            ">` to the Telegram bot.",
        );
      }
  ...
  const handleGenerateDiscordCode = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setDcSuccess(
          "Discord pairing code generated. Send `/pair code: " +
            data.pairingCode +
            "` to the Discord bot.",
        );
      }
          ...
          <div className="p-3 bg-[#0F0F0F] border border-[#262626] space-y-1.5">
            <div className="flex justify-between font-mono text-[10px] uppercase">
              ...
              <span className="text-[#FAFAFA] font-bold">
                {usageStats.storageUsed.toFixed(1)} MB /{" "}
                {usageStats.storageLimit} MB ({storagePct}%)
              </span>
            </div>
            ...
            <div className="space-y-6">
              <div className="bg-[#0F0F0F] border border-[#262626] p-5 space-y-4">
                ...
                <div className="space-y-3 text-xs font-mono">
                  <div className="flex justify-between pb-2 border-b border-[#1A1A1A]">
                    ...
                    <span className="text-[#FAFAFA] font-bold select-all">
                      {user.id}
                    </span>
                  </div>
                  <div className="flex justify-between pb-2 border-b border-[#1A1A1A]">
                  ...
                  <div className="flex justify-between pb-2 border-b border-[#1A1A1A]">
                    ...
                    <span className="text-[#FF3D00] font-black">
                      {user.role}
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-[#0F0F0F] border border-[#262626] p-5 space-y-3">
                ...
                <p className="text-xs text-[#737373] leading-relaxed">
                  Query or inject concept nodes programmatically into MindSpace
                  canvases from terminal scripts.
                </p>
          ...
          {activeTab === "info" && (
            <div className="space-y-4">
              <form
                ...
                className="bg-[#0F0F0F] border border-[#262626] p-5 space-y-4"
              >
                <h3 className="font-mono text-xs text-[#737373] uppercase tracking-wider flex items-center gap-1.5">
              ...
              {/* AI Engine & Privacy Toggle Card */}
              <div className="bg-[#0F0F0F] border border-[#262626] p-5 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-[#FF3D00]" />
                    <h3 className="font-mono text-xs text-[#FAFAFA] uppercase font-bold tracking-wider">
                      AI Features & Privacy Mode
                    </h3>
                  </div>
                  <button
                    type="button"
                    onClick={handleToggleAi}
                    className={`px-3 py-1 font-mono text-[10px] uppercase font-bold border transition-colors ${
                      isAiEnabled
                        ? "border-[#10B981] bg-[#10B981]/10 text-[#10B981]"
                        : "border-[#737373] bg-[#141414] text-[#737373]"
                    }`}
                  >
                    {isAiEnabled ? "AI Enabled" : "Privacy Mode (AI Off)"}
                  </button>
                </div>

                <p className="text-xs text-[#737373] leading-relaxed">
                  When disabled, external AI requests, background embeddings,
                  and copilot prompts are bypassed. MindSpace functions as a
                  private, local-first visual knowledge base.
                </p>
              </div>
            </div>
          )}
                ...
                {isUsageLoading ? (
                  ...
                  <div className="grid grid-cols-2 gap-3 text-center font-mono">
                    <div className="p-3 bg-[#141414] border border-[#262626]">
                      <div className="text-[10px] text-[#737373] uppercase">
                        Canvases
                      </div>
                      <div className="font-bold text-base text-[#FAFAFA]">
                        {usageStats.canvasCount}
                      </div>
                    </div>
                    <div className="p-3 bg-[#141414] border border-[#262626]">
                      <div className="text-[10px] text-[#737373] uppercase">
                    ...
                    <div className="p-3 bg-[#141414] border border-[#262626]">
                      ...
                      <div className="font-bold text-base text-[#FF3D00]">
                        {usageStats.storageUsed.toFixed(1)} MB
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === "webhooks" && (
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <h4 className="font-mono text-xs uppercase text-[#737373]">
                  Active Webhooks
                </h4>
                {isWebhooksLoading ? (
                  <div className="p-4 bg-[#0F0F0F] border border-[#262626] text-center text-[#737373] font-mono text-xs flex items-center justify-center gap-2">
                  ...
                  webhooks.map((wh) => (
                    <div
                      key={wh.id}
                      className="p-4 bg-[#0F0F0F] border border-[#262626] flex items-center justify-between"
                    >
                      <div>
              ...
              <div className="bg-[#0F0F0F] border border-[#262626] p-5 space-y-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-[#FAFAFA] font-bold uppercase tracking-wider">
                      Discord Integration
                    </span>
                    <span
                      className={`inline-block w-2 h-2 rounded-full ${discordAccount?.isPaired ? "bg-[#10B981]" : "bg-[#737373]"}`}
                    />
                  </div>
                  ...
                  {discordAccount?.isPaired ? (
                    <div className="p-4 bg-[#141414] border border-[#262626] space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[#FAFAFA] font-bold">
                          Linked as @{discordAccount.discordUsername || "User"}
                        </span>
                      </div>
                      <button
                    ...
                    <div className="p-4 bg-[#141414] border border-[#262626] space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        {discordAccount?.pairingCode ? (
                          <span className="font-bold text-sm text-[#FF3D00] select-all">
                            {discordAccount.pairingCode}
                          </span>
                        ) : (
                          <button
                ...
                <div className="space-y-3 border-t border-[#262626] pt-6">
                  <div className="flex justify-between items-center">
                    ...
                    <span
                      className={`inline-block w-2 h-2 rounded-full ${telegramAccount?.isPaired ? "bg-[#10B981]" : "bg-[#737373]"}`}
                    />
                  </div>
                  ...
                  {telegramAccount?.isPaired ? (
                    <div className="p-4 bg-[#141414] border border-[#262626] space-y-3">
                      <div className="flex justify-between items-center">
                        <span className="text-[#737373]">Status:</span>
                        <span className="text-[#FAFAFA] font-bold">
                          Linked as @{telegramAccount.username || "User"}
                        </span>
                      </div>
                      <button
